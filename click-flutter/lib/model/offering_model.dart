class OfferingModel {
  int error;
  String message;
  List<OfferingData> data;

  OfferingModel({this.error, this.message, this.data});

  OfferingModel.fromJson(Map<String, dynamic> json) {
    error = json['error'];
    message = json['message'];
    if (json['data'] != null) {
      data = new List<OfferingData>();
      json['data'].forEach((v) {
        data.add(new OfferingData.fromJson(v));
      });
    }
  }
...
class OfferingData {
  ...
  OfferingData.fromJson(Map<String, dynamic> json) {
    name = json['name'];
